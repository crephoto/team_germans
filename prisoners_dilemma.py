#If using < Python 3, uncomment the following line
...
>>>>>>> 270874a8602ffa8984f2a58f94f8a6bc8cef1a99
...
for f in ls():
    if "team_" in f:
        ...
        modules.append(my_mod)

#betray = example1
#collude = example0

# Add your team to the modules list below
...
def main_play(modules):
    ...
    now = time.localtime()
    outfile = "tournament_{:4d}-{:02d}-{:02d}_{:02d}{:02d}.txt".format(now.tm_year, now.tm_mon, now.tm_mday, now.tm_hour, now.tm_min)
    # DO NOT include team code listings in tournament.txt file
    post_to_file(section0+section1+section2+'CODE REMOVED'+''.join(section3), filename=outfile)
    # Comment the previous line and uncomment the following line to 
    # Include all teams' code in the section 3 reports.
...
def play_tournament(modules):
    ...
    for first_team_index in range(len(modules)):
        for second_team_index in range(first_team_index):
            ...
            # Python 3 automatically converts integer math to floats, so round the result
            scores[first_team_index][second_team_index] = round(score1/len(moves1)) # int division not an issue
            moves[first_team_index][second_team_index] = moves1
            # Redundant, but record this for the other player, from their perspective
            # Python 3 automatically converts integer math to floats, so round the result
            scores[second_team_index][first_team_index] = round(score2/len(moves2))
...
def play_round(player1, player2, score1, score2, moves1, moves2):
    ...
    if actions == 'cc':
        ...
        # Players get the "error score" if their code returns an improper action.
...
def make_section2(modules, scores):
    ...
    for index in range(len(modules)):
        section2_list.append((modules[index].team_name,
                              'P'+str(index),
                              str(int(sum(scores[index])/(len(modules)-1))), # Bots don't play against themselves
